...
void initialiseADC()
{
  ...
  if(configPage15.ADCFILTER_LPGP > 240) { configPage15.ADCFILTER_LPGP = 64; writeConfig(lpgSetPage); }
  if(configPage15.ADCFILTER_LPGTemp > 240) { configPage15.ADCFILTER_LPGTemp = 180; writeConfig(lpgSetPage); }
  vssCount = 0;
  vssTotalTime = 0;
}
...
byte getLpgp()
{
  if(configPage15.lpgpEnabled != 0 )
  {
    int tempReading;
    // readings
    #if defined(ANALOG_ISR_MAP)
      tempReading = AnChannel[pinLpgp-A0];
    #else
      tempReading = analogRead(pinLpgp);
      tempReading = analogRead(pinLpgp);
    #endif

    currentStatus.lpgpADC = ADC_FILTER(tempReading, configPage15.ADCFILTER_LPGP, currentStatus.lpgpADC); //Very weak filter

    currentStatus.lpgp = fastMap10Bit(currentStatus.lpgpADC, configPage15.lpgpMin, configPage15.lpgpMax); //Get the current MAP value
  }
}

void readLPGTemp()
{
  if(configPage15.lpgTempEnabled != 0 )
  {
    int tempReading;
    // readings
  #if defined(ANALOG_ISR)
    tempReading = fastMap1023toX(AnChannel[pinLPGTemp-A0], 511); //Get the current raw IAT value
  #else
    tempReading = analogRead(pinLPGTemp);
    tempReading = analogRead(pinLPGTemp);
    //tempReading = fastMap1023toX(analogRead(pinLPGTemp), 511); //Get the current raw IAT value
  #endif
  currentStatus.lpgTempADC = ADC_FILTER(tempReading, configPage15.ADCFILTER_LPGTemp, currentStatus.lpgTempADC);
  currentStatus.LPGTemp = table2D_getValue(&lpgTempCalibrationTable, currentStatus.lpgTempADC) - CALIBRATION_TEMPERATURE_OFFSET;
  }
}
