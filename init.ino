...
{   
    ...
    lpgpFuelTable.valueSize = SIZE_BYTE;
    lpgpFuelTable.axisSize = SIZE_BYTE;
    lpgpFuelTable.xSize = 8;
    lpgpFuelTable.values = configPage15.lpgpFuelValues;
    lpgpFuelTable.axisX = configPage15.lpgpFuelBins;
    LPGTempCorrectionTable.valueSize = SIZE_BYTE;
    LPGTempCorrectionTable.axisSize = SIZE_BYTE; //Set this table to use byte axis bins
    LPGTempCorrectionTable.xSize = 9;
    LPGTempCorrectionTable.values = configPage15.lpgTempValues;
    LPGTempCorrectionTable.axisX = configPage15.lpgTempBins;
...
void setPinMapping(byte boardID)
{
  ...
  if ( (configPage15.lpgpPin != 0) && (configPage15.lpgpPin < BOARD_NR_GPIO_PINS) ) { pinLpgp = configPage15.lpgpPin + A0; }
  if ( (configPage15.lpgTempPin != 0) && (configPage15.lpgTempPin < BOARD_NR_GPIO_PINS) ) { pinLPGTemp = configPage15.lpgTempPin + A0; }
...
// #endif
  ...
  {
    pinMode(pinLpgp, INPUT);
  }
  if(configPage15.lpgTempEnabled > 0)
  {
    pinMode(pinLPGTemp, INPUT);
  }
  if(configPage2.flexEnabled > 0)
  {
