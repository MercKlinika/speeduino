...
extern struct table2D LPGTempCorrectionTable; //9 bin lpg temperature density correction (2D)
extern struct table2D lpgpFuelTable; //8 bin lpg correction curve (2D)
...
#define pinIsSensor(pin)    ( ((pin) == pinCLT) || ((pin) == pinIAT) || ((pin) == pinMAP) || ((pin) == pinTPS) || ((pin) == pinO2) || ((pin) == pinBat) || ((pin) == pinLPGTemp) || ((pin) == pinLpgp) )
...
struct statuses {
  ...
  int LPGTemp;  
  int lpgTempADC;
  byte lpgTempCorrection; /**< The amount of lpg temperature adjustment currently being applied */
  byte lpgpCorrection; /**< The amount of correction being applied for the current baro reading */
};
...
//LPG Stuff
  int8_t lpgpMin; //Must be signed
  uint16_t lpgpMax;

  byte lpgTempBins[9];
  byte lpgTempValues[9];
  byte lpgpEnabled : 1;
  byte lpgTempEnabled : 1;
  byte lpgpPin : 4;
  byte lpgTempPin : 4;
  byte unused15_21 : 5; 

  byte ADCFILTER_LPGTemp;
  byte ADCFILTER_LPGP;

  byte lpgpFuelBins[8];
  byte lpgpFuelValues[8];
...
extern byte pinLPGTemp; // LPG Temp sensor pin
...
//extern byte lpgTempCalibrationTable[CALIBRATION_TABLE_SIZE]; /**< An array containing the lpg temperature sensor calibration values */
...
extern uint16_t lpgTempCalibration_bins[32];
extern uint16_t lpgTempCalibration_values[32];
...
extern struct table2D lpgTempCalibrationTable; /**< A 32 bin array containing the lpg temperature sensor calibration values */
...
static_assert(sizeof(struct config15) == 41, "configPage15 size is not 41");
#endif // GLOBALS_H
