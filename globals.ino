...
const uint16_t npage_size[NUM_PAGES] = {0,151,288,288,146,288,128,240,192,192,192,288,192,128,288,41}; /**< This array stores the size (in bytes) of each configuration page */
...
struct table2D lpgpFuelTable; //8 bin lpg correction curve (2D)
struct table2D LPGTempCorrectionTable; //9 bin lpg temperature density correction (2D)
...
//byte lpgTempCalibrationTable[CALIBRATION_TABLE_SIZE]; /**< An array containing the inlet air temperature sensor calibration values */
...
uint16_t lpgTempCalibration_bins[32];
uint16_t lpgTempCalibration_values[32];
struct table2D lpgTempCalibrationTable;
