...
void doUpdates()
{
  ...
  if(EEPROM.read(EEPROM_DATA_VERSION) == 9)
  {
    ...
    configPage15.ADCFILTER_LPGTemp = 180;    
    configPage15.ADCFILTER_LPGP= 64;
  ...
  if(EEPROM.read(EEPROM_DATA_VERSION) == 12)
  {
    ...
    //Manual lpg correction curve was added. Give it some default values (All lpg readings set to 100%)
    configPage15.lpgpFuelBins[0] = 80;
    configPage15.lpgpFuelBins[1] = 85;
    configPage15.lpgpFuelBins[2] = 90;
    configPage15.lpgpFuelBins[3] = 95;
    configPage15.lpgpFuelBins[4] = 100;
    configPage15.lpgpFuelBins[5] = 105;
    configPage15.lpgpFuelBins[6] = 110;
    configPage15.lpgpFuelBins[7] = 115;

    configPage15.lpgpFuelValues[0] = 100;
    configPage15.lpgpFuelValues[1] = 100;
    configPage15.lpgpFuelValues[2] = 100;
    configPage15.lpgpFuelValues[3] = 100;
    configPage15.lpgpFuelValues[4] = 100;
    configPage15.lpgpFuelValues[5] = 100;
    configPage15.lpgpFuelValues[6] = 100;
    configPage15.lpgpFuelValues[7] = 100;
  ...
  if(EEPROM.read(EEPROM_DATA_VERSION) == 14)
  {
    ...
    for(int x=0; x<(CALIBRATION_TABLE_SIZE/16); x++) //Each calibration table is 512 bytes long
    {
      ...
      y = EEPROM_CALIBRATION_LPGTemp_OLD + (x * 16);
      lpgTempCalibration_values[x] = EEPROM.read(y);
      lpgTempCalibration_bins[x] = (x * 32);
    }
    writeCalibration();

    //Lpg inputs were introduced. Disable them both by default
    configPage15.lpgpEnabled = false;
    configPage15.lpgTempEnabled = false;
