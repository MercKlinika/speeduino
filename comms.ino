  ...
  switch (currentCommand)
  {
    ...
    */
    ...
    case 'Z': //Totally non-standard testing function. Will be removed once calibration testing is completed. This function takes 1.5kb of program space! :S
      ...
      Serial.println(F("LPG temp"));
      for (int x = 0; x < 32; x++)
      {
        Serial.print(lpgTempCalibration_bins[x]);
        Serial.print(", ");
        Serial.println(lpgTempCalibration_values[x]);
      }
      Serial.println(F("O2"));
      for (int x = 0; x < 32; x++)
      {
...
void updateFullStatus()
{
  ...
  fullStatus[117] = (byte)(currentStatus.LPGTemp + CALIBRATION_TEMPERATURE_OFFSET); //lpg temp
  fullStatus[118] = currentStatus.lpgTempCorrection; //Lpg temperature Correction (%)
  fullStatus[119] = currentStatus.lpgp; //LPG pressure value
  fullStatus[120] = currentStatus.lpgpCorrection;
...
void sendValuesLegacy()
{
  ...
  temp = currentStatus.lpgTempCorrection * 10;
  bytestosend -= Serial.write(temp>>8); 
  bytestosend -= Serial.write(temp); 

  temp = currentStatus.lpgpCorrection * 10;
  bytestosend -= Serial.write(temp>>8); 
  bytestosend -= Serial.write(temp); 
...
void receiveValue(uint16_t valueOffset, byte newValue)
{
  ...
  switch (currentPage)
  {
    ...
    case lpgSetPage:
      pnt_configPage = &configPage15;
      //For some reason, TunerStudio is sending offsets greater than the maximum page size. I'm not sure if it's their bug or mine, but the fix is to only update the config page if the offset is less than the maximum size
      if (valueOffset < npage_size[lpgSetPage])
      {
        *((byte *)pnt_configPage + (byte)valueOffset) = newValue;
      }
      break;
...
void sendPageASCII()
{
  ...
  switch (currentPage)
  {
    ...
    case lpgSetPage:
      //currentTitleIndex = 141;
      //To Display Values from Config Page 15
      Serial.println((const __FlashStringHelper *)&pageTitles[103]);//special typecasting to enable suroutine that the F macro uses
      for (pnt_configPage = &configPage15; pnt_configPage < ( (byte *)&configPage15 + npage_size[lpgSetPage]); pnt_configPage = (byte *)pnt_configPage + 1)
      {
        Serial.println(*((byte *)pnt_configPage));// Displaying byte values of config page 9 up to but not including the first array
      }
      sendComplete = true;
...
void receiveCalibration(byte tableID)
{
  ...
  switch (tableID)
  {
    ...
    case 2:
     ...
     case 4:
      //Lpg temp table
      pnt_TargetTable_values = (uint16_t *)&lpgTempCalibration_values;
      pnt_TargetTable_bins = (uint16_t *)&lpgTempCalibration_bins;
      OFFSET = CALIBRATION_TEMPERATURE_OFFSET;
      DIVISION_FACTOR = 10;
      break;
