...
uint16_t correctionsFuel()
{
  ...
  currentStatus.lpgpCorrection = correctionLpgp();
  if (currentStatus.lpgpCorrection != 100) { sumCorrections = (sumCorrections * currentStatus.lpgpCorrection); activeCorrections++; }
  if (activeCorrections == MAX_CORRECTIONS) { sumCorrections = sumCorrections / powint(100,activeCorrections); activeCorrections = 0; } 

  currentStatus.lpgTempCorrection = correctionLPGTemp();
  if (currentStatus.lpgTempCorrection != 100) { sumCorrections = (sumCorrections * currentStatus.lpgTempCorrection); activeCorrections++; }
  if (activeCorrections == MAX_CORRECTIONS) { sumCorrections = sumCorrections / powint(100,activeCorrections); activeCorrections = 0; }
...
static inline byte correctionsFuel_new()
{
  ...
  currentStatus.lpgTempCorrection = correctionLPGTemp(); numCorrections++;
  currentStatus.lpgpCorrection = correctionLpgp(); numCorrections++; 
  ...
  sumCorrections = currentStatus.wueCorrection \
                  ...
                  + currentStatus.lpgTempCorrection \
                  + currentStatus.baroCorrection \
                  + currentStatus.lpgpCorrection \
...
/** Simple temperature based corrections lookup based on the lpg temperature (LPGTemp).
This corrects for changes in air density from movement of the temperature.
*/
byte correctionLPGTemp()
{
  byte LPGTempValue = 100;

  if (configPage15.lpgTempEnabled == 1)
  {
   LPGTempValue = table2D_getValue(&LPGTempCorrectionTable, currentStatus.LPGTemp + CALIBRATION_TEMPERATURE_OFFSET);
  }
  return LPGTempValue;
}
/**
...
/** Correction for current lpg pressure.
 * @returns A percentage value indicating the amount the fueling should be changed based on the barometric reading. 100 = No change. 110 = 10% increase. 90 = 10% decrease
 */
byte correctionLpgp()
{
  ...
  if (configPage15.lpgpEnabled == 1)
  {
    lpgpValue = table2D_getValue(&lpgpFuelTable, currentStatus.lpgp);
  }
  return lpgpValue;
}
/*
