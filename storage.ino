...
void writeAllConfig()
{
  ...
  if (eepromWritesPending == false) { writeConfig(lpgSetPage); }
}
  ...
  */
  ...
  case progOutsPage:
    ...
    case lpgSetPage:
      ...
      | 64 byte long config table
      -----------------------------------------------------*/
      pnt_configPage = (byte *)&configPage15; //Create a pointer to Page 2 in memory
      for(int x=EEPROM_CONFIG4_START; x<EEPROM_CONFIG15_END; x++)
      {
        if( (writeCounter > EEPROM_MAX_WRITE_BLOCK) ) { break; } //This is a safety check to make sure we don't attempt to write too much to the EEPROM at a time.
...
void loadCalibration()
{
  ...
  for(int x=0; x<32; x++) //Each calibration table is 32 bytes long
  {
    ...
    y = EEPROM_CALIBRATION_LPGTemp + (x * 2);
    EEPROM.get(y, lpgTempCalibration_bins[x]);
    y += 64; 
    EEPROM.get(y, lpgTempCalibration_values[x]);
...
void writeCalibration()
{
  ...
  for(int x=0; x<32; x++) //Each calibration table is 32 bytes long
  {
    ...
    y = EEPROM_CALIBRATION_LPGTemp + (x * 2);
    EEPROM.put(y, lpgTempCalibration_bins[x]);
    y += 64; 
    EEPROM.put(y, lpgTempCalibration_values[x]);
  }
